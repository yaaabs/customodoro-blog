      ...
      try {
        ...
        try {
          ...
          const anchors = Array.from(
            doc.querySelectorAll("blockquote a"),
          ) as HTMLAnchorElement[]
          const statusAnchor = anchors.find((a) =>
            /status\/.+/.test(a.getAttribute("href") || ""),
          )
          const dateLikeAnchor = anchors.find((a) =>
            /\d{1,2}:\d{2}\s*[AP]M|·|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec/i.test(
              a.textContent || "",
            ),
          )
          const anchor =
            statusAnchor || dateLikeAnchor || anchors[anchors.length - 1]
          if (anchor) {
            const txt =
              anchor.textContent || anchor.getAttribute("title") || undefined
    ...
    ;(async () => {
      const updated = await Promise.all(
        SOCIAL_UPDATES.map(async (s: Update) => {
          ...
          if (preview && !preview.dateText) {
            try {
              const api = await fetch(
                `/api/tweet-info?url=${encodeURIComponent(s.url || "")}`,
              )
              if (api.ok) {
                const apiData = await api.json()
                if (apiData && apiData.dateText)
                  preview.dateText = apiData.dateText
              }
            } catch {}
          }
                ...
                if (apiData && (apiData.description || apiData.dateText)) {
                  const esc = (str: string) =>
                    str
                      ...
                      .replace(/</g, "&lt;")
                      .replace(/>/g, "&gt;")
                  const contentHtml = apiData.description
                    ? `<p>${esc(apiData.description).replace(/\n/g, "<br/>")}</p>`
                    : undefined
                  return {
                    ...s,
                    loading: false,
          ...
          const handle = preview.authorUrl
            ? `@${preview.authorUrl
                .replace(/https?:\/\//, "")
                .split("/")
                .filter(Boolean)
          ...
          return {
            ...
            date: s.date, // manual override if provided
        ...
        {displayed.map((item) => (
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-medium leading-snug">
                    {item.author || (item.loading ? "Loading…" : "@X")}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {item.handle && <span className="mr-2">{item.handle}</span>}
                    <span>
                      {item.date || item.dateText || (item.loading ? "" : "")}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="text-sm text-foreground leading-relaxed">
              ...
              {item.failed && (
                <div className="text-sm text-muted-foreground">
                  Could not load preview — view on X
                </div>
              )}
      ...
      {visible.length > DISPLAY_LIMIT && (
        <div className="mt-6 flex justify-center">
          ...
          >
            {expanded
              ? "Show less"
              : `View more (${visible.length - DISPLAY_LIMIT} more)`}
          </button>
        </div>
      )}
    </section>
